...
class source_check(Command):
    # The so-called doc-string of the class will be visible in the built-in
    # help that is accessible by typing "?c" inside ranger.
    """:source_check <filename>
    Check that file exists before sourcing.
    """
    ...
    def execute(self):
        ...
        if self.arg(1):
            # self.rest(1) contains self.arg(1) and everything that follows
            target_filename = os.path.expanduser(self.rest(1))
        else:
            # self.fm is a ranger.core.filemanager.FileManager object and gives
            # you access to internals of ranger.
            # self.fm.thisfile is a ranger.container.file.File object and is a
        ...
        # This is a generic function to print text in ranger.  
        # self.fm.notify("Let's source the file " + target_filename + "!")
        ...
        # variety of subroutines that can help you construct commands.
        # Check out the source, or run "pydoc ranger.core.actions" for a list.
        self.fm.source(target_filename)
...
class fzf_locate(Command):
    ...
    def execute(self):
        import subprocess
        if self.quantifier:
            command="locate home media | fzf -i"
        else:
            command="locate home media | fzf -i"
        fzf = self.fm.execute_command(command, stdout=subprocess.PIPE)
        stdout, stderr = fzf.communicate()
